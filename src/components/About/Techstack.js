import React from "react";

function Techstack() {
  return (
    <section>
      <p>&nbsp;</p>
      {/* <p>&nbsp;</p> */}
      <h2 className="purple" style={{ fontVariant: "small-caps" }}>
        Operating Systems
      </h2>

      <p>
        <img
          src="https://cdn.simpleicons.org/windows/0078D6"
          height="70"
          alt="windows11 logo"
          title="Microsoft Windows 7/8/10/11 + Server 2012/2016/2019"
        />
        <img width="12" alt="" />
        <img
          src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/ubuntu/ubuntu-plain.svg"
          height="70"
          alt="ubuntu logo"
          title="Ubuntu"
        />
        <img width="12" alt="" />
        <img
          src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/debian/debian-original.svg"
          height="70"
          alt="debian logo"
          title="Debian"
        />
        <img width="12" alt="" />
        <img
          src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/opensuse/opensuse-original.svg"
          height="70"
          alt="opensuse logo"
          title="OpenSuse"
        />
        <img width="12" alt="" />
        <img
          src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg"
          height="70"
          alt="linux logo"
          title="Linux UI+CLI/Terminal"
        />
        <img width="12" alt="" />
        <img
          src="https://cdn.simpleicons.org/powershell/5391FE"
          height="70"
          alt="powershell logo"
          title="Microsoft Windows PowerShell 7"
        />
        <img width="12" alt="" />
        <img
          src="https://cdn.simpleicons.org/gnubash/4EAA25"
          height="70"
          alt="bash logo"
          title="Linux Bash (Terminal)"
        />
      </p>
      <br />
      <h2 className="purple" style={{ fontVariant: "small-caps" }}>
        Programming Languages
      </h2>
      <p>
        <div>
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg"
            height="70"
            alt="react logo"
          />
          <img width="12" alt="" />
          {/* <!--   <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="70" alt="csharp logo"  /> -->
<!--  <img width="12" alt="" /> --> */}
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg"
            height="70"
            alt="html5 logo"
            title="HTML 5"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg"
            height="70"
            alt="css3 logo"
            title="CSS 3"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sass/sass-original.svg"
            height="70"
            alt="sass logo"
            title="SASS / SCSS"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg"
            height="70"
            alt="javascript logo"
            title="JavaScript"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg"
            height="70"
            alt="typescript logo"
            title="TypeScript"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg"
            height="70"
            alt="nodejs logo"
            title="NodeJS"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.simpleicons.org/php/777BB4"
            height="70"
            alt="php logo"
            title="PHP 8"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
            height="70"
            alt="mysql logo"
            title="MySQL"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg"
            height="70"
            alt="mongodb logo"
            title="MongoDB"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.simpleicons.org/microsoftsqlserver/CC2927"
            height="70"
            alt="microsoftsqlserver logo"
            title="Microsoft SQL Server"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/markdown/markdown-original.svg"
            height="70"
            alt="markdown logo"
            title="MarkDown"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg"
            height="70"
            alt="bootstrap logo"
            title="BootStrap 5"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.simpleicons.org/tailwindcss/06B6D4"
            height="70"
            alt="tailwindcss logo"
            title="TailWind"
          />
          <img width="12" alt="" />
          <img
            src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/npm/npm-original-wordmark.svg"
            height="70"
            alt="npm logo"
            title="npm - Node Packet Manager"
          />
        </div>
      </p>
    </section>
  );
}

export default Techstack;
